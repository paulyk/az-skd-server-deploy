...
public class TestBase {
    ...
    public Lot Gen_Lot(Guid bomId, Guid pcvId, int kitCount = 6, bool assignVin = false) {
        ...
        var componentStations = context.ComponentStations
            .Include(t => t.Component)
            .Include(t => t.Station)
            .ToList();
        ...
        Kit GetKitForKitSeq(PCV pcv, int kitSeq) {
            Kit kit = Kit.Create(Gen_KitNo(lotNo, kitSeq));
            if (assignVin) {
    ...
    public void Gen_KitTimelineEventTypes() {
        ...
        var eventTypes = Enum.GetValues<KitStatusCode>()
            .Select((code, i) => new KitStatusEventType() {
                Code = code,
        ...
        foreach (var eventType in eventTypes) {
            if (!context.KitStatusEventTypes.Any(t => t.Code == eventType.Code)) {
                context.KitStatusEventTypes.AddRange(eventTypes);
            }
        }
        context.SaveChanges();
    }
