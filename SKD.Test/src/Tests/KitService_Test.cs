...
public class KitServiceTest : TestBase {
    ...
    public async Task Can_create_kit_timeline_events() {
        ...
        var timelineEvents = new List<(KitStatusCode eventType, DateTime trxDate, DateTime eventDate)>() {
                (KitStatusCode.CUSTOM_RECEIVED, baseDate.AddDays(0), baseDate.AddDays(-6)),
                (KitStatusCode.PLAN_BUILD,  baseDate.AddDays(0),baseDate.AddDays(2)),
                (KitStatusCode.BUILD_START, baseDate.AddDays(1), baseDate.AddDays(2)),
                (KitStatusCode.BUILD_COMPLETED,  baseDate.AddDays(5),baseDate.AddDays(5)),
                (KitStatusCode.GATE_RELEASED, baseDate.AddDays(10), baseDate.AddDays(10)),
                (KitStatusCode.WHOLE_SALE, baseDate.AddDays(12), baseDate.AddDays(12)),
            };
        ...
        var results = new List<MutationResult<KitStatusEvent>>();

        var before_count = context.KitStatusEvents.Count();

        foreach (var entry in timelineEvents) {
            var service = new KitService(context, entry.trxDate);
            var dto = new KitStatusEventInput {
                KitNo = kit.KitNo,
            ...
            if (entry.eventType == KitStatusCode.BUILD_START) {
            ...
            var result = await service.CreateKitStatusEventAsync(dto);
            results.Add(result);
        }

        // assert
        var after_count = context.KitStatusEvents.Count();
    ...
    public async Task Error_if_custom_receive_date_greater_than_current_date() {
        ...
        var input_1 = new KitStatusEventInput {
            KitNo = kit.KitNo,
            EventCode = KitStatusCode.CUSTOM_RECEIVED,
            EventDate = currentDate
        };

        var input_2 = new KitStatusEventInput {
            KitNo = kit.KitNo,
            EventCode = KitStatusCode.CUSTOM_RECEIVED,
        ...
        // test
        var result_1 = await service.CreateKitStatusEventAsync(input_1);
        var result_2 = await service.CreateKitStatusEventAsync(input_2);
    ...
    public async Task Cannot_create_kit_timeline_events_out_of_sequence() {
        ...
        var baseDate = DateTime.Now.Date;
        var timelineEvents = new List<(KitStatusCode eventType, DateTime trxDate, DateTime eventDate)>() {
                (KitStatusCode.CUSTOM_RECEIVED, baseDate.AddDays(1),  baseDate.AddDays(6)),
                (KitStatusCode.BUILD_COMPLETED, baseDate.AddDays(2), baseDate.AddDays(2)),
            };
        ...
        foreach (var (eventType, eventDate, trxDate) in timelineEvents) {
            var input = new KitStatusEventInput {
                KitNo = kit.KitNo,
                EventCode = eventType,
                EventDate = eventDate,
            };
            service = new KitService(context, trxDate);
            var result = await service.CreateKitStatusEventAsync(input);
            results.Add(result);
        }
        ...
        // assert
        var expectedMessage = "Missing status event";
        var actualMessage = lastPayload.Errors.Select(t => t.Message).FirstOrDefault();
        Assert.StartsWith(expectedMessage, actualMessage[..expectedMessage.Length]);
    }
    ...
    public async Task Create_kit_timeline_event_with_note() {
        ...
        var baseDate = DateTime.Now.Date;
        var timelineEventItems = new List<(KitStatusCode eventType, DateTime trxDate, DateTime eventDate, string eventNode)>() {
                (KitStatusCode.CUSTOM_RECEIVED, baseDate.AddDays(2), baseDate.AddDays(-appSettings.PlanBuildLeadTimeDays) , eventNote),
                (KitStatusCode.PLAN_BUILD, baseDate.AddDays(3), baseDate.AddDays(5), eventNote),
                (KitStatusCode.BUILD_START, baseDate.AddDays(4), baseDate.AddDays(5), eventNote),
                (KitStatusCode.BUILD_COMPLETED, baseDate.AddDays(8), baseDate.AddDays(8), eventNote),
                (KitStatusCode.GATE_RELEASED, baseDate.AddDays(10), baseDate.AddDays(10), eventNote),
                (KitStatusCode.WHOLE_SALE, baseDate.AddDays(11), baseDate.AddDays(11), eventNote),
            };
        ...
        // assert
        var timelineEvents = context.KitStatusEvents.ToList();
    ...
    public async Task Cannot_set_timline_event_date_to_future_date_for_build_complete_onwards() {
        ...
        var eventNote = Util.RandomString(15);
        var baseDate = DateTime.Now.Date;
        var timelineEventItems = new List<(KitStatusCode eventType, DateTime trxDate, DateTime eventDate, string expectedError)>() {
                (KitStatusCode.CUSTOM_RECEIVED, baseDate.AddDays(2), baseDate.AddDays(-appSettings.PlanBuildLeadTimeDays) , ""),
                (KitStatusCode.PLAN_BUILD, baseDate.AddDays(3), baseDate.AddDays(5), ""),
                (KitStatusCode.BUILD_START, baseDate.AddDays(4), baseDate.AddDays(5), ""),
                (KitStatusCode.BUILD_COMPLETED, baseDate.AddDays(8), baseDate.AddDays(9), "Date cannot be in the future"),
            };
    ...
    public async Task Create_kit_timline_event_removes_prior_events_of_the_same_type() {
        ...
        var dto2 = new KitStatusEventInput {
            KitNo = kit.KitNo,
            EventCode = KitStatusCode.CUSTOM_RECEIVED,
            EventDate = newDate
        };
        ...
        // test
        await service.CreateKitStatusEventAsync(dto);
        await service.CreateKitStatusEventAsync(dto2);
        ...
        var originalEntry = context.KitStatusEvents.FirstOrDefault(t => t.Kit.VIN == kit.VIN && t.RemovedAt != null);
        var latestEntry = context.KitStatusEvents.FirstOrDefault(t => t.Kit.VIN == kit.VIN && t.RemovedAt == null);
    ...
    public async Task Cannot_add_duplicate_kit_timline_event_if_same_type_and_date_and_note() {
        ...
        await service.CreateKitStatusEventAsync(dto);
        await service.CreateKitStatusEventAsync(dto2);
        var result = await service.CreateKitStatusEventAsync(dto2);

        // assert
        var after_count = context.KitStatusEvents.Count();
        Assert.Equal(2, after_count);
        var errorsMessage = result.Errors.Select(t => t.Message).First();
        var expectedMessage = "duplicate kit status event";
    ...
    public async Task Can_create_kit_timeline_event_by_lot() {
        ...
        var input = new LotKitStatusEventInput {
            LotNo = lot.LotNo,
            EventCode = KitStatusCode.CUSTOM_RECEIVED,
            EventDate = eventDate,
            EventNote = eventNote
        ...
        var result = await service.CreateLotKitStatusEventsAsync(input);
        Assert.Empty(result.Errors);

        var timelineEvents = context.KitStatusEvents.Where(t => t.Kit.Lot.LotNo == input.LotNo)
            .Include(t => t.Kit)
    ...
    public async Task Cannot_create_kit_timeline_event_by_lot_with_dupliate_date() {
        ...
        var result_2 = await service.CreateLotKitStatusEventsAsync(input);
        Assert.NotEmpty(result_2.Errors);

        var errorMessage = result_2.Errors.Select(t => t.Message).FirstOrDefault();
        var expectedMessage = "duplicate kit status event";
        var actualMessage = errorMessage[..expectedMessage.Length];
