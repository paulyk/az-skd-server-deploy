...
public class BomServiceTest : TestBase {
    ...
    public async Task Can_import_bom_file() {
        ...
        // only component stations in pcv.pcvComponents
        componentStations = componentStations
            .Where(cs => pcv.PcvComponents.Any(pcvComp => pcvComp.ComponentId == cs.ComponentId)).ToList();

        foreach (var kit in bom.Lots.First().Kits) {
            foreach (var mapping in componentStations) {
                var kitComponent = kit.KitComponents
                    .FirstOrDefault(t => t.ComponentId == mapping.ComponentId && t.ProductionStationId == mapping.StationId);
