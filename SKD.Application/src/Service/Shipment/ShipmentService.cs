...
public class ShipmentService: IAppService {
    ...
    public async Task<MutationResult<ShipmentOverviewDTO>> ImportShipment(ShipFile input) {
        ...
        MutationResult<ShipmentOverviewDTO> result = new() {
            Errors = await ValidateShipmentInput<ShipFile>(input)
        };
        ...
        var lots = await CreateUpdateLotAndKits(input, plant);

        // ensure shipment
        var shipment = await CreateUpdateShipment(input, plant, lots, parts);
    ...
    private async Task<List<Part>> CreateParts(ShipFile input) {
    ...
    private async Task<List<Lot>> CreateUpdateLotAndKits(ShipFile input, Plant plant) {
        ...
        foreach (var lotNo in lotNos) {
            ...
            if (lot == null) {
                ...
                var componentStations = context.ComponentStations.Where(cs => cs.RemovedAt == null);
                ...
                lot.Kits.AddRange(CreateKits(lotNo, pcv, componentStations));
            }
            lots.Add(lot);
        }
        return lots;
    ...
    private static  List<Kit> CreateKits(string lotNo, PCV pcv, IEnumerable<ComponentStation> componentStations) {
        var kits = new List<Kit>();
        for (int i = 1; i <= 6; i++) {
            var kitNo = $"{lotNo}0{i}";
            var kit = Kit.Create(kitNo);
    ...
    private async Task<Shipment> CreateUpdateShipment(ShipFile input, Plant plant, List<Lot> lots, List<Part> parts) {
    ...
    private async Task EnsureLotPartsCreated(ShipFile input, List<Lot> lots, List<Part> parts) {
        var lotPartInputs = GetLotPartInputsfromShipmentInput(input);
    ...
    public static List<LotPartQuantityDTO> GetLotPartInputsfromShipmentInput(ShipFile shipmentInput) {
