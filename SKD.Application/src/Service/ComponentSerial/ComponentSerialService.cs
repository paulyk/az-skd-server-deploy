...
public class ComponentSerialService : IAppService {
    ...
    public async Task<MutationResult<ComponentSerialDTO>> SaveComponentSerial(ComponentSerialInput input) {
        ...
        MutationResult<ComponentSerialDTO> result = new() {
            Errors = await ValidateSaveComponentSerial<ComponentSerialInput>(input with { })
        };
        ...
        var wasFormatted = formatSerialResult.Serials.Serial1 != input.Serial1 || formatSerialResult.Serials.Serial2 != input.Serial2;
        ...
        if (formatSerialResult.Serials.Serial1 != input.Serial1) {
            componentSerial.Original_Serial1 = input.Serial1;
        }
        if (formatSerialResult.Serials.Serial2 != input.Serial2) {
            componentSerial.Original_Serial2 = input.Serial2;
        }

        if (!wasFormatted) {
            // if componentSerial.Serial1 has any trailing control codes, remove them
            var newSerial1 = RemoveTrailingControlCodes(componentSerial.Serial1);
            if (newSerial1 != componentSerial.Serial1) {
                componentSerial.Serial1 = newSerial1;
                componentSerial.Original_Serial1 = input.Serial1;
            }
            // serial2
            var newSerial2 = RemoveTrailingControlCodes(componentSerial.Serial2);
            if (newSerial2 != componentSerial.Serial2) {
                componentSerial.Serial2 = newSerial2;
                componentSerial.Original_Serial2 = input.Serial2;
            }
        }
        ...
        // method to remove all trailing control codes from string
        static string RemoveTrailingControlCodes(string input) {
            var i = input.Length - 1;
            while (i >= 0 && char.IsControl(input[i])) {
                i--;
            }
            return input[..(i + 1)];
        }
    ...
    public async Task<List<Error>> ValidateSaveComponentSerial<T>(ComponentSerialInput input) where T : ComponentSerialInput {
        ...
        // kit.VIN cannot be null or empty
        if (String.IsNullOrWhiteSpace(kitComponent.Kit.VIN)) {
            errors.Add(new Error("VIN", $"kit.VIN cannot be null or empty: {kitComponent.Kit.KitNo}"));
            return errors;
        }
    ...
    public Error? ValidateComponentSerialRule(KitComponent kitComponent, ComponentSerialInput input) {
        ...
        // VIN is not allowed as serial if component.ComponentSerialRule != ComponentSerialRule.VIN_AND_BODY
        if (component.ComponentSerialRule != ComponentSerialRule.VIN_AND_BODY && component.ComponentSerialRule != ComponentSerialRule.VIN) {
            if (input.Serial1 == vin || input.Serial2 == vin) {
                return new Error("", $"Component {kitComponent.Component.Code} cannot have VIN as serial");
            }
        }
        ...
        switch (kitComponent.Component.ComponentSerialRule) {
            ...
            case ComponentSerialRule.VIN:
                return input.Serial1 != vin
                    ? new Error("", $"VIN required for component {kitComponent.Component.Code}")
                    : (Error?)null;
    ...
    public async Task<KitComponentSerialInfo?> GetKitComponentSerialInfo(string kitNo, string componentCode) {
        ...
        var result = new KitComponentSerialInfo {
            ...
            ComponentIconURL = data[0].IconURL,
