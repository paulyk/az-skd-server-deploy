...
public class PcvService : IAppService {
    ...
    public async Task<MutationResult<SavePcvPayload>> SavePCV(SavePcvInput input) {
        MutationResult<SavePcvPayload> payload = new() {
        ...
        PCV? pcv = await GetExistingPCV(input);
        ...
        await SetPcvMetadataProperties(input, pcv);
        ...
        if (input.ComponentCodes.Count == 0 && !pcv.PcvComponents.Any()) {
            PcvSubmodel? pcvSubmodel = await context.PcvSubmodels.FirstOrDefaultAsync(t => t.Code == input.PcvSubmodel);
            if (pcvSubmodel != null) {
                PCV? firstPcv = await context.Pcvs
        ...
        List<Component> selectedComponents = input.ComponentCodes is null 
            ? new List<Component>() 
        ...
        List<PcvComponent> pcvComponents = pcv.PcvComponents.ToList();
        foreach (PcvComponent pcvComponent in pcvComponents) {
    ...
    private async Task SetPcvMetadataProperties(SavePcvInput input, PCV pcv) {
    ...
    private async Task<PCV?> GetExistingPCV(SavePcvInput input) {
    ...
    public async Task<List<Error>> ValidateSavePcv(SavePcvInput input) {
        ...
        int modelYear = Int32.Parse(input.ModelYear);
    ...
    public async Task<MutationResult<DeletePcvPayload>> DeletePCV(DeletePcvInput input) {
        ...
        async Task RemovePcvMetaEntity<T>(T? entity) where T : EntityBase, IPcvMetaICategory, new() {
            if (entity != null) {
