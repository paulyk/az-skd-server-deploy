...
public class LotPartService: IAppService {
    ...
    public async Task<MutationResult<LotPartDTO>> CreateLotPartQuantityReceived(ReceiveLotPartInput input) {
        MutationResult<LotPartDTO> paylaod = new() {
            Errors = await ValidateCreateLotPartReceipt(input)
        };
        if (paylaod.Errors.Count > 0) {
            return paylaod;
        }

        LotPart lotPart = await context.LotParts
    ...
    public async Task<List<Error>> ValidateCreateLotPartReceipt(ReceiveLotPartInput input) {
        ...
        var lot = await context.Lots.AsNoTracking().FirstOrDefaultAsync(t => t.LotNo == input.LotNo);
        if (lot == null) {
            errors.Add(new Error("LotNo", $"lot not found {input.LotNo}"));
            return errors;
        }

        var part = await context.Parts.AsNoTracking().FirstOrDefaultAsync(t => t.PartNo == input.PartNo);
        if (part == null) {
        ...
        var lotPart = await context.LotParts.AsNoTracking()
            .Include(t => t.Received)
            .Where(t => t.Lot.LotNo == input.LotNo)
            .Where(t => t.Part.PartNo == input.PartNo)
            .FirstOrDefaultAsync();
    ...
    public async Task<List<LotPartReceivedDTO>> GetRecentLotPartsReceived(int count) {
        var result = await context.LotPartsReceived
            .OrderByDescending(t => t.CreatedAt)
            .Select(t => new LotPartReceivedDTO {
