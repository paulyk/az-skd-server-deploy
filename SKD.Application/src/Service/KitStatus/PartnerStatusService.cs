...
public class PartnerStatusService {
    ...
    private readonly KitStatusFeedService _kitStatusFeedService;
    private readonly KitService _kitService;
    private readonly ILogger<PartnerStatusService> _logger;
    ...
    public async Task<MutationResult<UpdatePartnerStatusPayload>> UpdatePartneStatusAsync(
        ...
        var kitStatusEventsToUpdated = await GetKitStatusEventsToSendAsync(input.KitNo, startStatusSquence);
        foreach (var kitStatusEvent in kitStatusEventsToUpdated) {
            var parnterStatusCode = (PartnerStatusCode)kitStatusEvent.EventType.Sequence;
        ...
        async Task<List<KitStatusEvent>> GetKitStatusEventsToSendAsync(string kitNo, int fromSequence) {
            return await _context.KitStatusEvents
    ...
    public async Task<KitProcessPartnerStatusRequest> GetProcessPartnerStatusInputAsync(
        ...
        var kit = await _context.Kits
                ...
                .Include(k => k.Dealer)
                .Include(k => k.KitStatusEvents.Where(e => e.RemovedAt == null))
        ...
        var kitStatusEvent = kit.KitStatusEvents
            .First(se => se.EventType.PartnerStatusCode == targetKitStatusCode);

        var planBuildStatusEvent = kit.KitStatusEvents
            .FirstOrDefault(t => t.EventType.Code == KitStatusCode.PLAN_BUILD);
    ...
    private async Task AssignNewVinToKitAsync(string kitNo, string vin) {
        _ = await _kitService.UpdateKitVinAsync(new KitVinInput {
            KitNo = kitNo,
            VIN = vin
        });
    }

    private static MutationResult<UpdateKitVinPayload> InitializeMutationResult(UpdateKitVinInput input, List<Error> errors) {
        return new MutationResult<UpdateKitVinPayload> {
    ...
    public async Task<MutationResult<UpdatePartnerStatusPayload>> SyncKitToPartnerStatusAsync(
        ...
        var orderedKitStatusEvents = kit.KitStatusEvents
    ...
    public async Task<List<Error>> ValidateSyncKitToPartnerStatusAsync(string kitNo) {
        ...
        var kit = await _context.Kits
            .Include(k => k.KitStatusEvents.Where(e => e.RemovedAt == null)).ThenInclude(t => t.EventType)
