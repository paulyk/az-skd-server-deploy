...
public class DCWSResponseService: IAppService {
    ...
    public async Task<MutationResult<DcwsResponse>> SaveDcwsComponentResponse(DcwsComponentResponseInput input) {
        MutationResult<DcwsResponse> result = new() {
            Errors = await ValidateDcwsComponentResponse<DcwsComponentResponseInput>(input)
        };
        if (result.Errors.Count > 0) {
            return result;
        }

        KitComponent kitComponent = await context.KitComponents
            .Include(t => t.ComponentSerials).ThenInclude(t => t.DcwsResponses)
            .Where(t => t.Id == input.KitComponentId)
            .FirstAsync();
    ...
    public async Task<List<Error>> ValidateDcwsComponentResponse<T>(DcwsComponentResponseInput input) where T : DcwsComponentResponseInput {
        ...
        if (kitComponent.RemovedAt != null) {
            errors.Add(new Error("", $"kit component removed for ComponentScanId = {input.KitComponentId}"));
            return errors;
        }
        ...
        if (componentSerial == null) {
            errors.Add(new Error("", $"component scan not found for ComponentScanId = {input.KitComponentId}"));
            return errors;
        }
