...
public class ComponentService : IAppService {
    ...
    public async Task<MutationResult<UpdateComponentPayload>> SaveComponent(ComponentInput input) {
        ...
        Component? component = await context.Components.FirstOrDefaultAsync(t => t.Id == input.Id);
    ...
    public static List<Error> ValidateRemoveComponent<T>(Component component) where T : Component { var errors = new List<Error>();
    ...
    public static List<Error> ValidateRestoreComponent<T>(Component component) where T : Component {
        var errors = new List<Error>();
