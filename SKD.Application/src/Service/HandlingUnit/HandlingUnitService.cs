...
public class HandlingUnitService: IAppService {
    ...
    public async Task<MutationResult<ReceiveHandlingUnitPayload>> SetHandlingUnitReceived(ReceiveHandlingUnitInput input) {
        ...
        MutationResult<ReceiveHandlingUnitPayload> result = new() {
            Errors = await ValidateSetHandlingUnitReceived(input)
        };
    ...
    public async Task<HandlingUnitInfoPayload?> GetHandlingUnitInfo(
        ...
        HandlingUnitInfoPayload result =
           await (from hu in context.HandlingUnits
                  ...
                  select new HandlingUnitInfoPayload {
                      ...
                      LotNo = lot.LotNo,
                      PcvCode = pcv.Code,
                      PcvDescription = pcv.Description,
        ...
        HandlingUnitReceived? received = await context.HandlingUnitReceived
            .OrderByDescending(t => t.CreatedAt)
            .Where(t => t.HandlingUnit.Code == code)
            .FirstOrDefaultAsync();

        if (received != null) {
            if (received.RemovedAt == null) {
                result.ReceivedAt = received.CreatedAt;
            } else {
                result.ReceivedRemovedAt = received.RemovedAt;
            }
        }
