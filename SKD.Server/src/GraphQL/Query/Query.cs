...
using SKD.Service.BOM;
using SKD.Application.ReceiveLotPart;
...
public class Query {
    ...
    public async Task<KitStatusDTO?> GetKitTimeline(
        ...
        var timelineEventTypes = await context.KitStatusEventTypes.AsNoTracking()
        ...
        var dto = new KitStatusDTO {
            ...
            TimelineItems = timelineEventTypes.Select(evtType => {
                var timelineEvent = kit.KitStatusEvents
    ...
    public async Task<LotOverviewDTO?> GetLotOverview(SkdContext context, string lotNo) {
        ...
        var timelineEvents = lot.Kits.SelectMany(t => t.KitStatusEvents);

        KitStatusEvent? customReceivedEvent = null;
        if (kit != null) {
            customReceivedEvent = kit.KitStatusEvents
                .OrderByDescending(t => t.CreatedAt)
                .Where(t => t.RemovedAt == null)
                .FirstOrDefault(t => t.EventType.Code == KitStatusCode.CUSTOM_RECEIVED);
        }
        ...
        return new LotOverviewDTO {
            ...
            CustomReceived = customReceivedEvent != null
                ? new StatusEventDTO {
                    EventType = KitStatusCode.CUSTOM_RECEIVED,
    ...
    public IQueryable<KitListItemDTO> GetKitList(
        ...
        string plantCode
            ...
            .Select(t => new KitListItemDTO {
                ...
                LastTimelineEvent = t.KitStatusEvents
                    .Where(t => t.RemovedAt == null)
                    .OrderByDescending(t => t.EventType.Sequence)
                    .Select(t => t.EventType.Description)
                    .FirstOrDefault(),
                LastTimelineEventDate = t.KitStatusEvents
                    .Where(t => t.RemovedAt == null)
                    .OrderByDescending(t => t.EventType.Sequence)
             ...
             await context.Lots.AsNoTracking()
                ...
                .Select(t => new LotListDTO {
                    ...
                    TimelineStatus = t.Kits
                        .SelectMany(t => t.KitStatusEvents)
                        .OrderByDescending(t => t.CreatedAt)
                        .Where(t => t.RemovedAt == null)
                        .Select(t => (KitStatusCode?)t.EventType.Code).FirstOrDefault(),
    ...
    public async Task<List<KitStatusEvent>> GetKitStatusEventsByDate(
        SkdContext context,
        string plantCode,
        DateTime fromDate,
        DateTime toDate,
        KitStatusCode? timelineEventCode
    ) {
        var query = context.KitStatusEvents
    ...
    public IQueryable<Kit> GetKitsByCurrentTimelineEvent(
        [Service] QueryService service,
        string plantCode,
        KitStatusCode? eventCode
    ) => service.GetKitsByCurrentKitStatusEvent(plantCode, eventCode);
    ...
    public async Task<LotPartDiffQueryResult> GetLotPartDiff(
        [Service] LotPartDiffService service,
        LotPartDiffInput input
